...
describe('DebateRoomService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: ConfigService, useValue: { get: jest.fn(() => 180) } },
      ],
    }).compile();
  ...
  describe('sendMessage', () => {
    ...
    it('STARTING 단계에서 발언하면 INVALID_PHASE 에러를 던진다', async () => {
  ...
  describe('next_turn 전체 시퀀스', () => {
    it('debateRoundCount=2일 때 OPENING×2 → FREETALKING×4 → CLOSING×2 → JUDGING 순서로 진행되고 발언자가 교대한다', async () => {
      ...
      await service.join(HOST, DEBATE_ID);
      await service.join(OPPONENT, DEBATE_ID); // STARTING → OPENING(host)
  ...
  describe('턴 제한시간 타이머', () => {
    it('제한시간이 지나면 자동으로 다음 턴으로 넘어간다', async () => {
      ...
      await service.join(HOST, DEBATE_ID);
      await service.join(OPPONENT, DEBATE_ID); // OPENING(host) 시작 + 타이머 예약
